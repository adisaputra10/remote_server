...
func (dp *DatabaseProxy) acceptConnections() {
	defer dp.agent.logger.Printf("🔚 Database proxy %s stopped", dp.Name)
	...
	for {
		...
		if err != nil {
			...
			dp.agent.logger.Printf("❌ Failed to accept connection on proxy %s: %v", dp.Name, err)
			continue
		}
...
func (dp *DatabaseProxy) handleConnection(clientConn net.Conn) {
	...
	dp.agent.logger.Printf("📡 New %s connection: %s (session: %s)", dp.Protocol, clientIP, sessionID)
	
	// Connect to target database
	targetAddr := fmt.Sprintf("%s:%d", dp.TargetHost, dp.TargetPort)
	targetConn, err := net.Dial("tcp", targetAddr)
	if err != nil {
		dp.agent.logger.Printf("❌ Failed to connect to target %s: %v", targetAddr, err)
		return
	}
	defer targetConn.Close()
	
	dp.agent.logger.Printf("✅ Connected to target %s", targetAddr)
	...
	<-done
	dp.agent.logger.Printf("🔚 Connection closed: %s", sessionID)
}
...
func (dp *DatabaseProxy) forwardWithInspection(src, dst net.Conn, direction, sessionID, clientIP string) {
	...
	for {
		...
		if err != nil {
			if err != io.EOF {
				dp.agent.logger.Printf("❌ Read error in %s (%s): %v", direction, sessionID, err)
			}
			return
		}
		...
		if err != nil {
			dp.agent.logger.Printf("❌ Write error in %s (%s): %v", direction, sessionID, err)
			return
		}
	}
}
...
func (dp *DatabaseProxy) inspectDatabaseCommands(data []byte, sessionID, clientIP string) {
	...
	if command != "" {
		dp.agent.logger.Printf("📝 DB_COMMAND [%s]: %s | %s | %s | %s", 
