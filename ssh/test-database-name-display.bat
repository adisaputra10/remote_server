@echo off
echo ===============================================
echo Testing Database Name Display in Dashboard
echo ===============================================
...
echo.
echo [Step 5] Running database test with database names...
echo This will generate queries with database.table references
go run .\test-db-golang.go

echo.
echo [Step 6] Checking agent logs for database name extraction...
echo.
echo === Agent Logs (last 20 lines) ===
type logs\AGENT-agent-linux.log | tail -20

echo.
echo === Relay Server Logs (last 10 lines) ===
type logs\server-relay.log | tail -10
...
echo.
echo ===============================================
echo Test completed. Check dashboard Database Queries for DATABASE column.
echo Look for database names like 'db1', 'test_users' etc.
echo ===============================================
pause