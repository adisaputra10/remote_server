-- Complete Database Migration for Token Authentication System (LOGS DATABASE)
-- Run this script if your Linux server uses 'logs' database instead of 'tunnel'
-- Date: 2025-09-21
...
SET @exist_users_token := (SELECT count(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA='logs' AND TABLE_NAME='users' AND COLUMN_NAME='token');
...
SET @exist_clients_token := (SELECT count(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA='logs' AND TABLE_NAME='clients' AND COLUMN_NAME='token');
...
SET @exist_clients_username := (SELECT count(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA='logs' AND TABLE_NAME='clients' AND COLUMN_NAME='username');
...
SET @exist_agents_token := (SELECT count(*) FROM information_schema.COLUMNS 
    WHERE TABLE_SCHEMA='logs' AND TABLE_NAME='agents' AND COLUMN_NAME='token');
